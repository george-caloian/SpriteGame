...
void Laborator8::Init()
{
...

	{
		...
		mesh->LoadMesh(RESOURCE_PATH::MODELS + "Primitives", "sun.obj");
		meshes[mesh->GetMeshID()] = mesh;
	}

	{
		Mesh* mesh = new Mesh("plane");
		//Mesh* mesh = CreatePlane("plane");
...
void Laborator8::Update(float deltaTimeSeconds)
{
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab8"], modelMatrix,glm::vec3(0,0,1));
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int mode = glGetUniformLocation(shader->program, "mode");
	glUniform1i(mode, this->mod);
